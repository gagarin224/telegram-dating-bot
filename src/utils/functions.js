module.exports = {
    validateConfig(config) {
        ...
        if (!min || min <= 0 || !Number(min)) {
            throw new Error(`Invalid configuration: min cannot be emptry, should be a number and greater than 0`);
        }

        if (!max || max <= 0 || !Number(max)) {
            throw new Error(`Invalid configuration: max cannot be emptry, should be a number and greater than 0`);
        }
    }
}